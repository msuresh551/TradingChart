...
{
    public partial class HighChartWithDesign : System.Web.UI.Page
    {
        ...
        protected void ddlscript_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                int i = 0;
                bool read = false;
                string url = "http://finance.google.com/finance/getprices?q=" + ddlscript.SelectedValue.ToUpper().ToString().Trim() + "&x=NSE&i=1800&p=2d&f=c&df=cpct&auto=1&ts=1266701290218";
                ...
                //closing
                url = "http://finance.google.com/finance/getprices?q=" + ddlscript.SelectedValue.ToUpper().ToString().Trim() + "&x=NSE&i=1800&p=1d&f=c&df=cpct&auto=1&ts=1266701290218";
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                int i = 0;
                bool read = false;
                string url = "http://finance.google.com/finance/getprices?q=" + txtScriptID.Text.ToUpper().ToString().Trim() + "&x=NSE&i=1800&p=2d&f=c&df=cpct&auto=1&ts=1266701290218";
                HttpWebRequest request = (HttpWebRequest)WebRequest.Create(url);
                HttpWebResponse response = (HttpWebResponse)request.GetResponse();
                String ver = response.ProtocolVersion.ToString();
                StreamReader reader = new StreamReader(response.GetResponseStream());
                ...
                //closing
                url = "http://finance.google.com/finance/getprices?q=" + txtScriptID.Text.Trim() + "&x=NSE&i=1800&p=1d&f=c&df=cpct&auto=1&ts=1266701290218";
                request = (HttpWebRequest)WebRequest.Create(url);
                response = (HttpWebResponse)request.GetResponse();
                ver = response.ProtocolVersion.ToString();
                reader = new StreamReader(response.GetResponseStream());
